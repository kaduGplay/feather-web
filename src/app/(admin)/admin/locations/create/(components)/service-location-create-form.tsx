...
export function ServiceLocationCreateForm() {
  ...
  async function onSubmit({ name, image, datacenter, identifier, is_active }: IFormData) {
    ...
    await apiClient
      ...
      .then(response => {
        toast.success('A localização foi criada com sucesso!');

        window.location.href = `/admin/locations/${response.data.id}`;
      })
      .catch(error => {
  ...
  return (
    <div className="lg:w-[800px] mx-auto bg-gray-200 dark:bg-dark-100 p-2 rounded-md">
      <Container>
        <h1 className="text-[22px] font-semibold my-8">Criando localização</h1>
